...
def main():
    ...
    blob_col = db[TXT_BLOBS_COLLECTION]
    state_col = db[STATE_COLLECTION]

    st = state_col.find_one({"_id": "txt"}) or {}
    ...
    out_dir = os.path.join(OUTPUT_ROOT, "txt", run_folder)
    ...
    with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as z:
        for m in meta_docs:
            ...
            file_name = m.get("file_name") or f"txt_{packed+1}.txt"
            ...
            # Prefer raw bytes if exist, else encode content_text
            content_raw = blob_doc.get("content_raw")
            content_text = blob_doc.get("content_text")

            if content_raw is not None:
                data_bytes = to_bytes(content_raw)
            elif content_text is not None:
                data_bytes = str(content_text).encode("utf-8")
            else:
                skipped += 1
                continue
            ...
            z.writestr(file_name, data_bytes)

            size_bytes = m.get("size_bytes") or len(data_bytes)
