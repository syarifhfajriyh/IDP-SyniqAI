...
# Try multiple possible locations for on_prem.env
script_dir = os.path.dirname(os.path.abspath(__file__))
env_paths = [
    os.path.join(script_dir, 'on_prem.env'),  # Same directory as script
    'on_prem.env',  # Current working directory
    os.path.join(os.getcwd(), 'on_prem.env')  # Explicit current directory
]
...
if not env_loaded:
    print(f"[WARNING] on_prem.env not found in: {env_paths}")
    print(f"[WARNING] Using default/empty credentials - authentication may fail")
...
if MONGO_URI:
    ...
    print(f"[INFO] MongoDB Configuration:")
    print(f"  Connection: Atlas/Custom URI")
    ...
    print(f"[INFO] Using MongoDB Atlas/Custom connection")
else:
    ...
    print(f"[INFO] MongoDB Configuration:")
    print(f"  Connection: On-Prem")
    print(f"  Host: {MONGO_HOST}:{MONGO_PORT}")
    print(f"  Database: {DB_NAME}")
    print(f"  Auth Source: {MONGO_AUTH_SOURCE}")
    print(f"  User: {MONGO_USER if MONGO_USER else '(not set)'}")
    ...
    if MONGO_USER and MONGO_PASS:
        ...
        print(f"[INFO] Using authenticated on-prem connection")
    else:
        MONGO_URI = f"mongodb://{MONGO_HOST}:{MONGO_PORT}/"
        print(f"[WARNING] No credentials provided - using unauthenticated connection")

BLOBS_COLLECTION = "images_blobs"
META_COLLECTION = "images_metadata"
...
# - Recommended: 1000 for images (balance between performance and memory)
...
def main():
    ...
    file_paths = filedialog.askopenfilenames(
        title="Select images to upload (multi-select - up to 1000 files)",
        filetypes=[("Images", "*.jpg *.jpeg *.png *.tiff *.bmp *.webp"), ("All files", "*.*")]
    )
    if not file_paths:
        print("[ERROR] No images selected.")
        return

    print(f"\n[INFO] Starting upload of {len(file_paths)} images...")
    ...
    db = client[DB_NAME]
    blobs = db[BLOBS_COLLECTION]
    meta = db[META_COLLECTION]
    state = db[STATE_COLLECTION]

    extracted_at = utc_now_iso()
    run_id = extracted_at  # ✅ use extracted_at as run_id (simple)
    ...
    def flush():
        ...
        try:
            if blob_ops:
                res = blobs.bulk_write(blob_ops, ordered=False)
                inserted_blob += res.inserted_count
            if meta_ops:
                res2 = meta.bulk_write(meta_ops, ordered=False)
    ...
    for idx, path in enumerate(file_paths, 1):
        ...
        try:
            ...
            if checksum in seen:
                dup += 1
                if idx % 100 == 0:
                    print(f"  [PROGRESS] Processed {idx}/{len(file_paths)} files (in-memory duplicate: {name[:30]})")
                continue
            seen.add(checksum)

            # Check if blob exists globally - if yes, skip both blob AND metadata
            t_db = time.time()
            if blobs.find_one({"checksum": checksum}, {"_id": 1}):
                db_time += time.time() - t_db
                dup += 1
                if idx % 100 == 0:
                    print(f"  [PROGRESS] Processed {idx}/{len(file_paths)} files (DB duplicate: {name[:30]})")
                continue
            db_time += time.time() - t_db
    ...
    print("\n" + "="*70)
    print("UPLOAD COMPLETE - IMAGE INGESTION SUMMARY")
    print("="*70)
    ...
    if inserted_blob > 0 and total_time > 0:
        ...
        old_total = file_read_time + checksum_time + old_db_time + old_connection_time + old_duplicate_checks
